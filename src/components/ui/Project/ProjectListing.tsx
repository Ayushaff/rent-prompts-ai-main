...
const ProductList = () => {
  ...
  useEffect(() => {
    const getRapps = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/privateRapps/getRapps`);
        const result = await response.json();
        setData(result?.data?.myRapps);
      } catch (error) {
  ...
  const handleRun = (e: React.MouseEvent, slug: string) => {
    e.preventDefault();
    handleActionClick(e);
    window.open(`/dashboard/rapps/run/${slug}`, "_blank");
  };

  const handleEdit = (id: string, e: React.MouseEvent) => {
    e.preventDefault();
    handleActionClick(e);
    toast.success(`Editing Rapp with id: ${id}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    if (deleteRappId) {
      const deleteRapp = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/privateRapps/deleteRapp`, {
        method: 'DELETE',
        headers: {
    ...
    <div className="rounded-sm border border-stroke bg-indigo-800 shadow-default dark:border-strokedark dark:bg-boxdark sm:p-6">
      ...
      {loading ? (
        ...
        <div className="justify-center mx-auto mt-5 py-10 h-96 bg-indigo-900/[0.4] border-dashed border-2 border-muted-foreground rounded-lg flex items-center flex-col">
          <p className="text-gray-400">No Rapps available. Want to create a new one?</p>
          <Link href="/dashboard/rapps/create/private">
            ...
            >
              <div className="overflow-hidden flex items-center gap-3 sm:gap-4">
                ...
                <div className="text-xs sm:text-base break-words whitespace-normal">
                  <h5 className="font-medium text-white break-word whitespace-normal">{rapp.rappName}</h5>
                  <p className="text-gray-500 dark:text-gray-400 line-clamp-2 break-words whitespace-normal">{rapp.rappDes}</p>
                </div>
              </div>
              <p className="text-xs sm:text-sm text-right text-white">${rapp.cost}</p>
...
>
  {rapp.rappStatus === "approved" && <p className="flex flex-row gap-1 items-center"><Icons.approved /> <span className="hidden md:block">Approved</span></p> }
  {rapp.rappStatus === "pending" && <p className="flex flex-row gap-1 items-center"><Icons.pending /> <span className="hidden md:block">Pending</span></p>}
  {rapp.rappStatus === "denied" && <p className="flex flex-row gap-1 items-center"><Icons.denied /> <span className="hidden md:block">Denied</span></p>}
</p>
      ...
      {deleteRappId && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-md shadow-lg p-6 dark:bg-gray-800">
            ...
            <p className="text-gray-600 dark:text-gray-300">Are you sure want to delete this Rapp?</p>
